...
)
{
            ...
            if(type != null)
                transaction {
                    ...
                    val results = MemberDataTable
                            ...
                            .slice(type.column, MemberDataTable.userId)
                            .select { MemberDataTable.guildId.eq(event.guild.idLong) }

                            // Formats the stat list
                            .map { row -> MutablePair(row[MemberDataTable.userId], row[type.column]) }
    ...
    {
        MESSAGE_COUNT(Regex("(message|msg)s?"), MemberDataTable.messageCount, "message")
        {
        ...
        TEXT_CHAT(Regex("t(ext)?c(hat)?"), MemberDataTable.textChatTime, "textchat")
        {
            override fun format(guild: Guild, data: List<MutablePair<Long, Any?>>): List<String>
            {
                @Suppress("UNCHECKED_CAST")
        ...
        VOICE_CHAT(Regex("v(oice)?c(hat)?"), MemberDataTable.voiceChatTime, "voicechat")
        {
            override fun format(guild: Guild, data: List<MutablePair<Long, Any?>>): List<String>
            {
                @Suppress("UNCHECKED_CAST")

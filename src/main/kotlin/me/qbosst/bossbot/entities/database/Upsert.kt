...
fun <T: Table> T.upsert(vararg keys: Column<*>, body: T.(InsertStatement<Number>) -> Unit) =
        UpsertStatement<Number>(this, keys = keys)
                .apply {
                    body(this)
...
fun <T: Table, E> T.batchUpsert(data: Collection<E>, vararg keys: Column<*>, body: T.(BatchUpsertStatement, E) -> Unit)
{
    ...
    BatchUpsertStatement(this, keys = keys)
            .apply {
                data.forEach {
                    addBatch()
                    body(this, it)
...
class UpsertStatement<Key : Any>(table: Table, isIgnore: Boolean = false, private vararg val keys: Column<*>):
...
class BatchUpsertStatement(table: Table, isIgnore: Boolean = false, private vararg val keys: Column<*>):
